...
export default function MobileHeader({
  ...
  return (
    <header className="bg-f1-card/90 backdrop-blur-xl border-b border-f1-border sticky top-0 z-50 safe-top">
        ...
        <div className="flex items-center gap-2.5 min-w-0">
          ...
          <div className="min-w-0">
            ...
            >
              ...
              <svg className={`w-3 h-3 text-f1-dim flex-shrink-0 ${showPicker ? "rotate-180" : ""}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <div className="flex gap-1.5 px-3 pb-2 overflow-x-auto no-scrollbar">
        {availableTypes.map((type) => {
          ...
          return (
            <button
              key={type}
              onClick={() => { match && onSessionChange(match.key); setShowPicker(false); }}
      ...
      {/* Full GP picker overlay */}
      {showPicker && (
        <div className="fixed inset-0 top-0 z-[100] bg-f1-bg/95 backdrop-blur-sm flex flex-col">
          {/* Picker header */}
          <div className="flex items-center justify-between px-4 py-3 border-b border-f1-border">
            ...
            <button
              ...
              className="text-f1-dim p-1 active:text-white"
            >
          ...
          <div className="flex-1 overflow-y-auto">
            {(!meetings || meetings.length === 0) ? (
              <div className="px-4 py-8 text-center text-f1-dim text-sm">
...
function MobileGPRow({ meeting, isActive, onSelect }) {
  ...
  return (
    <div className={`border-b border-f1-border/40 ${isActive ? "bg-f1-elevated" : ""}`}>
      ...
      {expanded && (
        <div className="bg-f1-bg px-4 pb-3">
          {loading ? (
            <div className="text-xs text-f1-dim py-2">Loading...</div>
